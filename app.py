...
from sqlalchemy import Integer, and_, case, cast, create_engine, func
...
from models.transactions import TeamOnWaivers, WaiverPriority
...
config = {"CACHE_TYPE": "simple", "CACHE_DEFAULT_TIMEOUT": 5}
...
# Configure the database with Neon-compatible pooling
...
# Initialize schema on first request instead of import time
@app.before_request
def init_schema():
                ...
                [
                    {
                        ...
                        "team_name": team.fantasy_team_name,
                    }
                    for team in teams
                ]
            )
    ...
    try:
        with Session() as session:
            ...
            waiver_teams_query = (
                ...
                .join(TeamOnWaivers, Team.team_number == TeamOnWaivers.team_number)
                .join(TeamScore, Team.team_number == TeamScore.team_key)
                .join(FRCEvent, TeamScore.event_key == FRCEvent.event_key)
                .filter(TeamOnWaivers.league_id == leagueId, FRCEvent.year == year)
                .order_by(
                    cast(
                        Team.team_number, Integer
                    ).asc()  # Cast team_number to Integer for sorting
                )
                .all()
            )
            ...
            for row in waiver_teams_query:
                ...
                waiver_teams[team_number]["events"].append(
                    {"event_key": event_key, "week": week}
                )
...
def get_rosters(leagueId):
    ...
    try:
        with Session() as session:
            ...
            teamsOwnedInLeague = session.query(TeamOwned).filter(
                TeamOwned.league_id == leagueId
            )
            output = []
            for team in teams:
                output.append(
                    {
                        ...
                        "roster": [
                            frcteam.team_key
                            for frcteam in teamsOwnedInLeague.filter(
                                TeamOwned.fantasy_team_id == team.fantasy_team_id
                            ).all()
                        ],
                    }
                )
...
def get_roster_weeks(leagueId):
    ...
    try:
        with Session() as session:
            ...
            for team in teams:
                team_roster = teamsOwnedInLeague.filter(
                    TeamOwned.fantasy_team_id == team.fantasy_team_id
                ).all()
                roster_output = []
                ...
                for frcteam in team_roster:
                    ...
                    roster_output.append(
                        {"team_key": frcteam.team_key, "events": event_details}
                    )
    ...
    try:
        with Session() as session:
            ...
            draft_picks = (
                ...
                .filter(DraftPick.draft_id == draftId)
                .order_by(DraftPick.pick_number.asc())
                .all()
            )

            draft: Draft = (
                session.query(Draft).filter(Draft.draft_id == draftId).first()
            )
            ...
            for pick in draft_picks:
                ...
                if league.is_fim:
                    events = [
                        {"event_key": event.event_key, "week": event.week}
...
def get_draft_order(draftId):
    ...
    try:
        with Session() as session:
            ...
            draft_order = (
                session.query(DraftOrder)
                .filter(DraftOrder.draft_id == draftId)
                .order_by(DraftOrder.draft_slot.asc())
                .all()
            )
    ...
    try:
        with Session() as session:
            ...
            started_teams = (
                ...
                .filter(
                    TeamStarted.fantasy_team_id.in_(
                        [ft.fantasy_team_id for ft in fantasy_teams]
                    )
                )
                .all()
            )
            ...
            for week in weeks_to_render:
                ...
                for fantasy_team in fantasy_teams:
                    ...
                    for started_team in started_teams:
                        if (
                            started_team.fantasy_team_id == fantasy_team.fantasy_team_id
                            and started_team.week == week
                        ):
...
def get_fantasy_scores(leagueId, week):
    ...
    try:
        with Session() as session:
            ...
            fantasy_scores = (
                session.query(FantasyScores)
                .filter(FantasyScores.league_id == leagueId, FantasyScores.week == week)
                .order_by(FantasyScores.fantasy_team_id.asc())
                .all()
            )
            ...
            for score in fantasy_scores:
                ...
                fantasy_team = (
                    session.query(FantasyTeam)
                    .filter(FantasyTeam.fantasy_team_id == score.fantasy_team_id)
                    .first()
                )
                ...
                for started_team in started_teams:
                    ...
                    team_score = (
                        ...
                        .filter(
                            ...
                            TeamScore.event_key
                            == started_team.event_key,  # Assuming the event_key relates to the TeamScore table
                        )
                        .first()
                    )
    ...
    try:
        with Session() as session:
            ...
            waiver_priority = (
                ...
                .join(
                    FantasyTeam,
                    WaiverPriority.fantasy_team_id == FantasyTeam.fantasy_team_id,
                )
                .filter(WaiverPriority.league_id == leagueId)
            ...
            if not waiver_priority:
                return (
                    jsonify({"error": "League not found or no waiver priorities"}),
                    404,
                )
...
def get_league_rankings(leagueId):
    ...
    try:
        with Session() as session:
            ...
            result.sort(
                key=lambda x: (x["total_ranking_points"], x["tiebreaker"]), reverse=True
            )
        ...
        with Session() as session:
            ...
            if isFiM:
                base_query = (
                    ...
                    .outerjoin(
                        StatboticsData,
                        (Team.team_number == StatboticsData.team_number)
                        & (StatboticsData.year == previous_year),
                    )
                    .filter(
                        FRCEvent.year == year,
            ...
            for row in result:
                ...
                week = row.week
                year_end_epa = (
                    row.year_end_epa if row.year_end_epa is not None else 0
                ...
                available_teams[team_number]["events"].append(
                    {"event_key": event_key, "week": week}
                )
        ...
        with Session() as session:
            ...
            available_teams_query = (
                ...
                .outerjoin(
                    TeamOwned,
                    and_(
                        Team.team_number == TeamOwned.team_key,
                        TeamOwned.league_id == leagueId,
                    ),
                )
                .outerjoin(TeamOnWaivers, Team.team_number == TeamOnWaivers.team_number)
                .filter(
                    TeamOwned.team_key.is_(None),  # Teams not on a fantasy team
                    TeamOnWaivers.team_number.is_(None),  # Teams not on waivers
                    FRCEvent.year
                    == year,  # Teams registered for an event in the current year
...
def get_team_avatar(team_number, year):
    ...
    try:
        ...
        return jsonify(
            {"team_number": team_number, "image": avatarImage["details"]["base64Image"]}
        )
    except:
        return jsonify({"team_number": team_number, "image": None})
...
def get_team_epas():
    ...
    with Session() as session:
        query = session.query(
            StatboticsData.team_number, StatboticsData.year_end_epa
    ...
    try:
        with Session() as session:
            ...
            subquery = (
                ...
                .join(FRCEvent, TeamScore.event_key == FRCEvent.event_key)
                .filter(StatboticsData.year == 2026, FRCEvent.year == 2027)
                .subquery()
            )
            ...
            query = (
                session.query(
                    ...
                    func.max(
                        case((subquery.c.rank == 8, subquery.c.epa), else_=None)
                    ).label("top8epa"),
                    func.max(
                        case((subquery.c.rank == 24, subquery.c.epa), else_=None)
                    ).label("top24epa"),
                    func.avg(subquery.c.epa).label("avgepa"),
                    func.percentile_cont(0.5)
                    .within_group(subquery.c.epa)
                    .label("medianepa"),
                )
